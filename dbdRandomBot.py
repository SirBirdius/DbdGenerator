...
async def setKill(*args):
	...
	offerChoices = ['Cut Coin','Scratched Coin','Moldy Oak','Rotten Oak','Nothing','Clear Reagent','Hazy Reagent','Murky Reagent','New Moon Bouquet','Crescent Moon Bouquet','Nothing','Full Moon Bouquet','Shroud of Separation','Cypress Memento Mori','Ivory Memento Mori','Nothing','Putrid Oak','Quarter Moon Bouquet','Ebony Memento Mori']
	await client.say( 'Offering: ' + random.choice(offerChoices))

	
	perkChoices = ['A Nurse\'s Calling','Agitation','Barbecue and Chilli','Beast of Prey','Bitter Murmer','Bloodhound','Blood Warden','Brutal Strength','Deerstalker','Distressing','Dying Light','Enduring','Fire Up','Franklin\'s Demise','Insidious','Iron Grasp','Hangman\'s Trick','Hex: Devour Hope','Hex: Huntress Lullaby','Hex: No One Escapes Death','Hex: Ruin','Hex: The Third Seal','Hex: Thrill of the Hunt','Knock Out','Lightborn','Make Your Choice','Monitor and Abuse','Monstrous Shrine','Overcharge','Overwhelming Presence','Remember Me','Play With Your Food','Predator','Save The Best For Last','Shadowborn','Sloppy Butcher','Spies From The Shadows','Stridor','Surveillance','Territorial Imperative','Tinkerer','Thanatophobia','Unnerving Presence','Unrelenting','Whispers','Bamboozle','Coulrophobia','Pop Goes The Weasel','Nothing','Nothing','Nothing','Nothing','Nothing','Nothing','Nothing','Nothing','Spirit Fury','Hex: Haunted Ground','Rancor']

	perk1 = random.choice(perkChoices)
	await client.say('Perk 1: ' + perk1);
	perk2 = random.choice(perkChoices)
	while (perk2 == perk1 and perk2 != 'Nothing'):
		
		perk2 = random.choice(perkChoices)
	await client.say('Perk 2: ' + perk2)
	perk3 = random.choice(perkChoices)
	while(perk3 == perk2 and perk3 != 'Nothing' or perk3 == perk1 and perk3 != 'Nothing'):
		
		perk3 = random.choice(perkChoices)
	await client.say('Perk 3: ' + perk3)
	...
	if(killer == 'Trapper'):
		...
		while (addon2 == addon1 and addon2 != 'Nothing'):
			addon2 = random.choice(addonList)
		await client.say('Item addon 1: ' + addon1)
		await client.say('Item addon 2: ' + addon2)
	...
	if(killer == 'Spirit'):
		addontList = ['Zori','Shiawase Amulet','Origami Crane','Gifted Bamboo Comb','White Hair Ribbon','Rin\'s Broken Watch','Muddy Sports Day Cap','Kaiun Talisman','Juniper Bonsai','Rusty Flute','Katsumori Talisman','Katana Tsuba','Dirty Uwabaki','Bloody Hair Brooch','Yakuyoke Amulet','Wakizashi Saya','Prayer Beads Bracelet','Dried Cherry Blossom','Mother-Daughter Ring','Father\'s Glasses','Nothing','Nothing','Nothing']
		addon1 = random.choice(addonList)
		addon2 = random.choice(addonList)
		while (addon2 == addon1 and addon2 != 'Nothing'):
			addon2 = random.choice(addonList)
...
async def setSur(*args):
	...
	await client.say('Survivor: ' + random.choice(survChoices))
	offerChoices = ['Petrified Oak','Clear Reagent','Faint Reagent','Hazy Reagent','Nothing','New Moon Bouquet','Crescent Moon Bouquet','Shroud of Union','Vigo\'s Shroud','Shroud of Binding','Quarter Moon Bouquet','Nothing','Chalk Pouch','Cream Chalk Pouch','Ivory Chalk Pouch','Salt Pouch','Black Salt Statuette','Nothing','Tarnished Coin','Shiny Coin','Murky Reagent','Full Moon Bouquet','Vigo\'s Jar of Salty Lips']
	await client.say( 'Offering: ' + random.choice(offerChoices))



	perkChoices = ['Adrenaline','Balanced Landing','Bond','Botany Knowledge','Nothing','Dark Sense','Decisive Strike','Deja Vu','Empathy','Nothing','Left Behind','Lightweight','No One Left Behind','Object of Obsession','Plunderer\'s Instinct','Premonition','Prove Thyself','Nothing','Resilience','Unbreakable','Self-Care','Slippery Meat','Small Game','Sole Survivor','Nothing','Sprint Burst','Borrowed Time','This is not Happening','Stake Out','We\'ll Make It','Iron Will','Nothing','Open Handed','Up The Ante','Leader','Calm Spirit','Hope','Nothing','Spine Chill','Ace In The Hole','Kindred','Detective\'s Hunch','Nothing','Quick And Quiet','Technician','Alert','Lithe','Saboteur','We\'re Gonna Live Forever','Dead Hard','No Mither','Nothing','Pharmacy','Vigil','Wake Up!','Tenacity','Streetwise','Urban Evasion','Nothing','Dance With Me','Windows Of Opportunity','Boil Over','Diversion','Deliverance','Autodidact','Nothing']
...
@client.command()
async def poke(*args):
    await client.send_message(message.author, "boop")
